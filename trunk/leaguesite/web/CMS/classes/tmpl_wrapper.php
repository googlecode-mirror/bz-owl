<?php
	...
	class tmpl_wrapper
	{
		...
		static function display($file)
		{
			...
			$tmpl->display($file . '.tmpl.html');
		}
		
		static function setTemplate($template, $customTheme='')
		{
			global $config;
			global $user;
			global $tmpl;
			...
			self::findTemplate($template, $themeFolder);
			
			// init template system
			$tmpl->setTemplateDir($themeFolder . 'templates');
			$tmpl->setCompileDir($themeFolder . 'templates_c');
			$tmpl->setCacheDir($themeFolder . 'cache');
			$tmpl->setConfigDir($themeFolder . 'config');

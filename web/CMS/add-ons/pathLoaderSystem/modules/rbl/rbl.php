...
if (!defined('RBL_DIRECTORY')) {define('RBL_DIRECTORY', dirname(__FILE__));}
	...
	if ($proxylisted < 1)
	{
		...
		foreach ($opmlist as $OPM)
		{
			...
			if (preg_match("[njabl.org]", $OPM, $matches))
			{
				...
				{
					if (isset($opmfound[$OPM]) && $opmfound[$OPM]['result'] == $value && $block[$njcheck] > 0) {$blockremote = $opmfound[$OPM]['result'];}
				}
			}
			...
			if (preg_match("[efnet.org]", $OPM, $matches))
			{
				...
				{
					if (isset($opmfound[$OPM]['result']) && $opmfound[$OPM]['result'] == $value && $block[$efcheck] > 0) {$blockremote = $opmfound[$OPM]['result'];}
				}
			}
...
return $blockremote;
	...
	if ($blockremote)
	{
		// Add your things here, or just use the current error message from the template.
		if (defined('BLACKLISTED_USE_TEMPLATE'))
		{
			eval('$blacklisted = "' . template(BLACKLISTED_USE_TEMPLATE) . '";');
			echo $blacklisted;
			exit;
		}
		if (defined('BLACKLISTED_REDIRECT'))
		{
			header("Location: " . BLACKLISTED_REDIRECT	, 0, 301);
		}

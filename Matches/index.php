...
<?php
	...
	function team_name_from_id($site, $connection, $id)
	{
		...
		{
			$site->dieAndEndPage('The name of matching team with id ' . $row['team1_teamid'] . ' could not be displayed because of an SQL/database connectivity problem.');
		}
		
		// no rows in team name query result is not valid
		if ((int) mysql_num_rows($result_team_name) === 0)
		{
			echo '<span style="font-weight: bold">No team name for id ' . $row['team1_teamid'] . '!</span>' . "\n";
